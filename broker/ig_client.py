...
class IGClient:
    ...
    def login(self):
        ...
        r = self.s.post(f"{self.base}/session", json={"identifier": self.username, "password": self.password}, headers=hdr, timeout=20)
        r.raise_for_status()
        self.h = {
            "X-IG-API-KEY": self.api_key,
    ...
    def market_details(self, epic: str):
        r = self.s.get(f"{self.base}/markets/{epic}", headers=self._hv("3"), timeout=20)
        r.raise_for_status(); return r.json()

    def get_prices(self, epic: str, resolution="MINUTE", max=200):
        r = self.s.get(f"{self.base}/prices/{epic}", params={"resolution": resolution, "max": max}, headers=self._hv("3"), timeout=25)
        r.raise_for_status(); return r.json()
    ...
    def place_order(self, epic, direction, size, stop_distance=None, limit_distance=None, trailing=None, tif="FILL_OR_KILL"):
        ...
        r = self.s.post(f"{self.base}/positions/otc", json=payload, headers=self._hv("2"), timeout=25)
        r.raise_for_status(); return r.json()

    def close_position(self, deal_id, direction, size):
        payload = {"dealId": deal_id, "direction": direction, "size": size, "orderType": "MARKET"}
        r = self.s.post(f"{self.base}/positions/otc/close", json=payload, headers=self._hv("1"), timeout=20)
        r.raise_for_status(); return r.json()

    def positions(self):
        r = self.s.get(f"{self.base}/positions", headers=self._hv("2"), timeout=20)
        r.raise_for_status(); return r.json()

    def account_summary(self):
        r = self.s.get(f"{self.base}/accounts", headers=self._hv("1"), timeout=20)
        r.raise_for_status(); return r.json()